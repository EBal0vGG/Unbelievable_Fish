...
func (r *AuctionRepository) load(ctx context.Context, id app.AuctionID, forUpdate bool) (*auction.Auction, error) {
	query := `
SELECT auction_id, lot_id, state, starts_at, ends_at, current_price, leader_company_id
...
`
	...
	if err := row.Scan(&auctionID, &lotID, &state, &startsAt, &endsAt, &currentPrice, &leaderCompany); err != nil {
