...
deal_id="$(docker exec -i "$PG_CONTAINER" psql -U "$PGUSER" -d "$PGDATABASE" -t -A -c "select deal_id from deal_winner_selections where auction_id = '$auction_id';")"
deal_id="$(echo "$deal_id" | tr -d '[:space:]')"
if [[ -z "$deal_id" ]]; then
  echo "Deal not found for auction: $auction_id" >&2
  exit 1
fi
